...
def balance():
    ...
    return jsonify({'message': 'Balance fetched', 'result': result})


@app.route('/get_dfkc_amount')
def dfkc_amount():
    result = get_dfkc_amount_sync()
    return jsonify({'message': 'DFKC amount fetched', 'result': result})


@app.route('/get_klay_locked')
def klay_locked():
    result = get_klay_locked_sync()
    if result is not None:
        return jsonify({'message': 'Klay locked amount fetched', 'result': result})
    else:
        return jsonify({'message': 'Error fetching Klay locked amount', 'result': None})
